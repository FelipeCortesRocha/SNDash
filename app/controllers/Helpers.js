...
const negationWords = [' nao ', ' n ', ' nop ', ' not ', ' nada ']
let positiveWords = []
let negativeWords = []
let str
...
const removeStopWords = str => {
    ...
    let regex_str
    let regex
    let cleansed_string = str
    let stop_words = [...stopWordsPT.split(',')].concat([...stopWordsEN.split(',')])
...
const checkWord = word => {
    if (Array.isArray(word.portuguese)) {
        word.portuguese.forEach(w => {
            if (str.indexOf(w) != -1) return word;
        })
    }
    if (Array.isArray(word.english)) {
        word.english.forEach(w => {
            if (str.indexOf(w) != -1) return word;
        })
    }
    if (Array.isArray(word.spanish)) {
        word.spanish.forEach(w => {
            if (str.indexOf(w) != -1) return word;
        })
    }
    if (str.indexOf(word.portuguese) != -1 || str.indexOf(word.english) != -1 || str.indexOf(word.spanish) != -1) return word;
    return false;
}
...
const checkSentiment = tweetString => {
    ...
    let positives = positiveWords.filter(word => {
        return checkWord(word);
    });
    let negatives = negativeWords.filter(word => {
        return checkWord(word);
    });

    let totalWordsFound = positives.length + negatives.length;

    if (totalWordsFound > 0) console.log(str);
